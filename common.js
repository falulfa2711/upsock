...
function initProductSlider(categoryName) {
    ...
    // גלילה אוטומטית כל 3 שניות, עצירה בעת ריחוף עכבר
    let autoTimer = setInterval(() => _sliderTick(), 3000);
    slider.addEventListener('mouseenter', () => clearInterval(autoTimer));
    slider.addEventListener('mouseleave', () => { autoTimer = setInterval(() => _sliderTick(), 3000); });
}

function _sliderTick() {
    const s = document.getElementById('productSlider');
    if (!s) return;
    const maxScroll = s.scrollWidth - s.clientWidth;
    if (maxScroll <= 0) return;
    if (s.scrollLeft >= maxScroll - 5) {
        // הגענו לסוף — קפיצה לתחילה ללא אנימציה
        s.scrollLeft = 0;
    } else {
        s.scrollBy({ left: 185, behavior: 'smooth' });
    }
}
...
function slideProducts(dir) {
    const s = document.getElementById('productSlider');
    if (!s) return;
    // direction: ltr על הסליידר → next גולל ימינה (+), prev גולל שמאלה (-)
    s.scrollBy({ left: dir === 'next' ? 185 : -185, behavior: 'smooth' });
}
