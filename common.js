...
function injectCommonCSS() {
    ...
    style.textContent = `
        ...
        .product-slider { display: flex; gap: 15px; overflow-x: auto; scroll-behavior: smooth; padding: 10px 5px 15px; scrollbar-width: none; -ms-overflow-style: none; }
...
function _sliderTick() {
    ...
    if (s.scrollLeft <= 0) {
        ...
        s.scrollBy({ left: -200, behavior: 'smooth' });
    }
}
...
function slideProducts(dir) {
    const s = document.getElementById('productSlider');
    if (!s) return;
    s.scrollBy({ left: dir === 'next' ? -220 : 220, behavior: 'smooth' });
}
