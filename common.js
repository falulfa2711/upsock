...
function injectCommonCSS() {
    ...
    style.textContent = `
        ...
        .slider-inner { display: flex; gap: 15px; width: max-content; animation: sliderMarquee 30s linear infinite; }
        .product-slider:hover .slider-inner { animation-play-state: paused; }
        .product-slider .product-card { min-width: 170px; max-width: 170px; flex-shrink: 0; }
...
function initProductSlider(categoryName) {
    ...
    // כפל התוכן + עטיפה ב-slider-inner — הגלילה מופעלת ע"י CSS בלבד
    slider.innerHTML = `<div class="slider-inner">${cardsHtml + cardsHtml}</div>`;
}

// ===== גלילה אוטומטית — מופעלת ע"י CSS Animation בלבד =====
// אין צורך ב-JS לגלילה — CSS @keyframes sliderMarquee מטפל בהכל
function _startSliderAuto() { /* CSS מטפל */ }
function _stopSliderAuto()  { /* CSS :hover מטפל */ }

function slideProducts(dir) {
    const s     = document.getElementById('productSlider');
    const inner = s && s.querySelector('.slider-inner');
    if (!inner) return;
    // קריאת מיקום נוכחי מהאנימציה ← הזזה ידנית
    const x = new DOMMatrix(getComputedStyle(inner).transform).m41;
    inner.style.animation = 'none';
    inner.style.transform  = `translateX(${x + (dir === 'next' ? -185 : 185)}px)`;
    clearTimeout(s._rt);
    // חזרה לגלילה אוטומטית אחרי 3 שניות
    s._rt = setTimeout(() => {
