...
function injectCommonCSS() {
    ...
    style.textContent = `
        ...
        .product-slider { display: flex; gap: 15px; overflow-x: auto; padding: 10px 5px 15px; scrollbar-width: none; -ms-overflow-style: none; direction: ltr; }
        .product-slider::-webkit-scrollbar { display: none; }
        .product-slider .product-card { min-width: 170px; max-width: 170px; flex-shrink: 0; direction: rtl; }
...
function syncSearch(val) {
    ...
    const filtered   = allProducts.filter(p => {
        ...
        if (queryTerms.length > 1) {
            return queryTerms.every(termOrSynMatch);
        }
...
const SLIDER_PRODUCTS = {
    'חד פעמי':              ['6923538238643','729120320314','0835811000419','0835811000758','0835811001515','835811002659','0835811004493','0835811005872'],
    ...
    'מוצרי אפיה ובישול':    ['0835811000396','0835811000730','0835811000853','0835811001119','0835811002246','0835811004257'],
...
function initProductSlider(categoryName) {
    ...
    slider.innerHTML = cardsHtml + cardsHtml; // כפל — לגלילה חלקה ואינסופית

    // גלילה רציפה וחלקה ב-requestAnimationFrame
    _startSliderAuto();
    slider.addEventListener('mouseenter', _stopSliderAuto);
    slider.addEventListener('mouseleave', _startSliderAuto);
}

// ===== מנוע גלילה אוטומטית (RAF — 60fps, ללא לאג) =====
let _sliderRAF  = null;
let _sliderPrev = null;
const SLIDER_SPEED = 50; // פיקסלים לשנייה

function _startSliderAuto() {
    _stopSliderAuto();
    _sliderPrev = null;
    function _frame(ts) {
        if (!_sliderPrev) _sliderPrev = ts;
        const dt   = ts - _sliderPrev;
        _sliderPrev = ts;
        const s    = document.getElementById('productSlider');
        if (s) {
            const half = s.scrollWidth / 2;
            if (half > 0 && s.scrollLeft >= half) {
                s.scrollLeft -= half;   // קפיצה חלקה חזרה לתחילה
            } else {
                s.scrollLeft += SLIDER_SPEED * dt / 1000;
            }
        }
        _sliderRAF = requestAnimationFrame(_frame);
    }
    _sliderRAF = requestAnimationFrame(_frame);
}

function _stopSliderAuto() {
