...
// ===================================================
// ⚙️  הגדרות תשלום ויצירת קשר
// ===================================================
const WHATSAPP_NUMBER = '972549397753';
const BIT_NUMBER      = '0543191550';
const PAYBOX_NUMBER   = 'YOUR_PAYBOX_NUMBER'; // עדכן כשיהיה חשבון PayBox
...
// ===================================================
// 🏗️  בניית HTML משותף (header + nav + modals)
// ===================================================
function injectCommonHTML() {
    ...
    header.innerHTML = `
        ...
        <div class="search-box-mini">
            <input type="text" id="mainSearchInput" placeholder="🔍 חפש מוצר..." onkeyup="syncSearch(this.value)">
        </div>`;
    ...
    imgOverlay.innerHTML = '<img id="viewImg" src="">';
...
function extractPrice(priceValue) {
    ...
    // אם זה טקסט מבצע (כולל אותיות/רווחים כמו "3 ב-10") — מחזיר את המספר האחרון
    const nums = str.match(/\d+(\.\d+)?/g);
    if (!nums) return 0;
    // אם יש יותר ממספר אחד (כמו "3 ב-10") — קח את האחרון (המחיר האמיתי)
    return parseFloat(nums[nums.length - 1]);
}

function isPromoPrice(priceValue) {
    // בדיקה אם זה טקסט מבצע ולא מחיר רגיל
    const str = String(priceValue);
    return isNaN(parseFloat(str)) || str.includes('ב') || str.includes('ב-') || /\d+\s/.test(str);
}
...
function updateUI() {
    ...
    const itemsHtml = cart.map((i, idx) => {
        let actualPrice = extractPrice(i.price);
        let qty = i.qty || 1;
        let lineTotal = actualPrice * qty;
        count += qty; total += lineTotal;
        // חישוב מחיר לפריט מבצע
        let lineTotal, priceNote;
        if (i.isPromo && i.unitPrice && i.promoPrice && i.units) {
            const totalUnits = qty * i.units;          // סה"כ יחידות
            const fullPromos = Math.floor(totalUnits / i.units);  // מארזים שלמים
            const remainder  = totalUnits % i.units;              // יחידות בודדות
            lineTotal  = (fullPromos * i.promoPrice) + (remainder * i.unitPrice);
            const breakdown = fullPromos > 0 ? `${fullPromos} מארז ×₪${i.promoPrice}` : '';
            const remPart   = remainder > 0 ? `${remainder} יח' ×₪${i.unitPrice}` : '';
            priceNote = `<div style="font-size:0.75em;color:#888;">${[breakdown, remPart].filter(Boolean).join(' + ')}</div>`;
        } else if (i.isPromo && i.promoPrice) {
            lineTotal = qty * i.promoPrice;
            priceNote = `<div style="font-size:0.75em;color:#888;">מארז ×${qty}</div>`;
        } else {
            lineTotal = extractPrice(i.price) * qty;
            priceNote = '';
        }
        const unitsLabel = i.units ? ` (${i.units} יח' במארז)` : '';
        return `<div class="cart-item">
            ...
            <div style="text-align:right;">
                <strong>${i.name}</strong><small style="color:#888;">${unitsLabel}</small>
                ${priceNote}
            </div>
        </div>`;
    }).join('');
...
function getOrderDetails() {
    ...
    let total = 0;
    currentCart.forEach(i => { total += extractPrice(i.price) * (i.qty || 1); });
...
function buildWhatsAppMsg() {
    ...
    currentCart.forEach(i => {
        ...
        const isPromo  = priceStr.includes('ב') || /[א-ת]/.test(priceStr) || (priceStr.match(/\d+/g) || []).length > 1;
        if (isPromo) {
            msg += `• ${i.name}\n  מבצע: ${i.price} | כמות: ${q}\n`;
        } else {
...
// ===================================================
// 🚀  אתחול — נקרא אוטומטית בטעינת כל דף
// ===================================================
document.addEventListener('DOMContentLoaded', function() {
    ...
    // שמירה אוטומטית בהקלדה
    document.getElementById('cust-name').addEventListener('input',  e => localStorage.setItem('cust_name',  e.target.value));
    document.getElementById('cust-phone').addEventListener('input', e => localStorage.setItem('cust_phone', e.target.value));
